...
let goodSet = [1, 2, 3, 3, 4, 10]
let evilSet = [1, 2, 2, 2, 3, 5, 10]

func evaluate(good: String, vsEvil evil: String) -> String {
    let goodScore = zip(goodSet, good.split(separator: SEPARATOR).compactMap { Int($0) })
        .map { $0 * $1 }
        .reduce(0, +)
    let evilScore = zip(evilSet, evil.split(separator: SEPARATOR).compactMap { Int($0) })
        .map { $0 * $1 }
        .reduce(0, +)
    switch goodScore - evilScore {
    case 0: return "Battle Result: No victor on this battle field"
    case ..<0: return "Battle Result: Evil eradicates all trace of Good"
...
class SolutionTest: XCTestCase {
    ...
    func testGoodShouldTriumph() {
      XCTAssertEqual("Battle Result: Good triumphs over Evil", evaluate(good: "0 0 0 0 0 10", vsEvil: "0 1 3 1 1 0 0"))
    }
